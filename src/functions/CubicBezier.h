...
public:
  ...
  void setControlPoints(const vectorN &x0, const vectorN &x1, const vectorN &x2,
      const vectorN &x3) {
    unsigned int length = x0.getLength();

    if (length <= 0 || x1.getLength() != length || x2.getLength() != length
        || x3.getLength() != length)
      throw std::invalid_argument(
          "Control points length should match and be greater than zero");
  ...
  vectorN evaluate(real t) const {
    if (t < 0 || t > 1.0)
      throw std::invalid_argument(
          "t parameter must belong to [0.0, 1.0] interval");
...
//      logger->debug("result = [%s]", result.toString().c_str());
    ...
    return x0 * pow(1.0 - t, 3.0) + x1 * (3.0 * pow(1.0 - t, 2.0) * t)
        + x2 * (3.0 * (1.0 - t) * pow(t, 2.0)) + x3 * pow(t, 3.0);
  }
